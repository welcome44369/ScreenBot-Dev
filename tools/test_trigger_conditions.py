...
def observation(state, reason=None):
    return ObservationResult(state, state == "PRESENT", 1.0 if state == "PRESENT" else 0.0, .9, None, .9, state != "INVALID", reason or state, "target" if state == "PRESENT" else "", "target")


def feed(trigger, states):
    return [trigger.update(observation(state), now=index + 1).triggered for index, state in enumerate(states)]
...
def main():
    ...
    assert feed(trigger({"mode": "edge", "desired_state": "absent"}), ["PRESENT", "ABSENT"]) == [False, True]
    memory = {}
    ...
    legacy = TextTrigger("target", event="appear", confirm_frames=1)
    assert feed(legacy, ["PRESENT"]) == [True]

    # Recovery: two qualified partial-PRESENT attempts lost back to ABSENT
    # create one token, consumed by the following static ABSENT confirmation.
    ...
    assert recovery.memory["lost_episode_count"] == 1
    ...
    assert recovery.memory["recovery_token"] == 1
    assert recovery.update(observation("ABSENT"), now=6).triggered
    assert recovery.memory["recovery_token"] == 0
    assert not recovery.update(observation("ABSENT"), now=7).triggered
